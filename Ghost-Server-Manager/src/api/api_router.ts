...
router.post("/create", async (req, res) => {
	logger.info({ label: "create_instance", message: "Route called" });

	// if (!req.query.hasOwnProperty("port")) {
	// 	logger.info({ label: "create_instance", message: "No port in query. Exiting." });
	// 	res.status(400).send("Port is required!");
	// 	return;
	// }

	// let wsPort: number = 53000;
	// if (req.query.hasOwnProperty("ws_port")) {
	// 	wsPort = +req.query.ws_port.toString();
	// }

	const ports = await getAllColumnValues<number>("port");
	if (ports.length >= MAX_NUMBER_OF_GHOST_SERVERS) {
		logger.info({ label: "create_instance", message: "Max number of concurrent ghost servers. Cancelling." });
		res.status(507).send();
		return;
	}
	...
	logger.info({ label: "create_instance", message: `Starting container with port: ${port} and wsPort: ${wsPort}...` });
	...
	logger.info({ label: "create_instance", message: "Container ready. Requesting websocket start..." });

	await axios.get(`http://localhost:${port}/startServer`);

	logger.info({ label: "create_instance", message: "Container successfully started" });
...
function waitForContainerStart(container: Docker.Container): Promise<void> {
	return new Promise((resolve, reject) => {
		container.attach({ stream: true, stdout: true }, (err, stream) => {
			if (err) {
				console.log(`Failed to attach to container ${container.id}: ${err}`);
				reject(err);
				return;
			}

			const listener = async (chunk) => {
				logger.info({ label: "waitForContainerStart", message: `Container data: ${chunk}` });
...
router.get("/delete", containerAuthMiddleware, async (req, res) => {
	logger.info({ label: "delete", message: "Route called" });

	const container = JSON.parse(req.query.container.toString());
	await deleteContainer(container.id);
	await stopContainer(container.containerId, container.port);

	logger.info({ label: "delete", message: "Successfully stopped and deleted the container" });
	res.status(200).send();
});
